...
class ModDetails(_SashDetailsPanel):
    ...
    bsaAndVoice = _('This mod has an associated archive (%s.' +
                    bush.game.bsa_extension + ') and an '
        'associated voice directory (Sound\\Voices\\%s), which will become '
        'detached when the mod is renamed.') + '\n\n' + _('Note that the '
        'BSA archive may also contain a voice directory (Sound\\Voices\\%s), '
        'which would remain detached even if the archive name is adjusted.')
    bsa = _('This mod has an associated archive (%s.' +
                    bush.game.bsa_extension + '), which will become '
        'detached when the mod is renamed.') + '\n\n' + _('Note that this '
        'BSA archive may contain a voice directory (Sound\\Voices\\%s), which'
        ' would remain detached even if the archive file name is adjusted.')
    voice = _('This mod has an associated voice directory (Sound\\Voice\\%s),'
    ...
    def ShowBashTagsMenu(self):
        ...
        class _TagLink(CheckLink):
            def _initData(self, window, selection):
                ...
                self.help = _("Add %(tag)s to %(modname)s") % (
...
class ModPanel(BashTab):
    ...
    def _sbCount(self): return _('Mods:') + ' %d/%d' % (
...
class _Tab_Link(AppendableLink, CheckLink, EnabledLink):
    ...
    def __init__(self,tabKey,canDisable=True):
        ...
        self.help = _("Show/Hide the %(tabtitle)s Tab.") % (
            {'tabtitle': self._text})
...
class BashFrame(BaltFrame):
    ...
    def SetTitle(*args, **kwargs):
        ...
        if CBashApi.Enabled:
            ...
            title += ': '
        maProfile = re.match(r'Saves\\(.+)\\$',bosh.saveInfos.localSave,re.U)
        if maProfile:
