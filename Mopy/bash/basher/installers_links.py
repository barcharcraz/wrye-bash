...
class Installers_AddMarker(ItemLink):
    ...
    _text = _('Add Marker...')
    help = _('Adds a Marker, a special type of package useful for separating'
...
class Installers_MonitorInstall(Installers_Link):
    ...
    help = _('Monitors the Data folder during installation via manual install'
             ' or 3rd party tools.')
...
class Installers_ListPackages(Installers_Link):
    ...
    help = _('Displays a list of all packages.  Also copies that list to the '
...
class Installers_AnnealAll(Installers_Link):
    """Anneal all packages."""
    _text = _('Anneal All')
    help = _('This will install any missing files (for active installers)'
...
class Installers_UninstallAllPackages(Installers_Link):
    ...
    help = _('This will uninstall all packages.')
...
class Installers_UninstallAllUnknownFiles(Installers_Link):
    ...
    _text = _('Clean Data')
    help = _('This will remove all mod files that are not linked to an'
             ' active installer out of the Data folder.')
    fullMessage = _("Clean Data directory?") + "  " + help + "  " + _(
...
class Installers_AutoAnneal(BoolLink):
    _text, key, help = _('Auto-Anneal'), 'bash.installers.autoAnneal', _(
...
class Installers_AutoWizard(BoolLink):
    ...
    help = _("Enable/Disable automatic installing or anneal (as applicable)"
...
class Installers_WizardOverlay(_Installers_BoolLink_Refresh):
    ...
    help =_("Enable/Disable the magic wand icon overlay for packages with"
            " Wizards.")
...
class Installers_AutoApplyEmbeddedBCFs(ItemLink):
    ...
    help = _('Automatically apply Embedded BCFs to their containing archives.')
...
class Installers_AutoRefreshBethsoft(BoolLink, Installers_Link):
    ...
    help = _('Skip installing Bethesda ESMs, ESPs, and BSAs')
    opposite = True
...
class Installers_Enabled(BoolLink):
    ...
    _text, key, help = _('Enabled'), 'bash.installers.enabled', _(
...
class Installers_BsaRedirection(AppendableLink, BoolLink, EnabledLink):
    ...
    help = _("Use Quarn's BSA redirection technique.")

    def _initData(self, window, selection):
        super(Installers_BsaRedirection, self)._initData(window, selection)
        if not self._enable():
            self.help += '  ' + _('%(ini)s must exist') % {
...
class Installers_ConflictsReportShowsInactive(_Installers_BoolLink_Refresh):
    ...
    help = _('In the conflicts tab also display conflicts with inactive (not '
             'installed) installers')
...
class Installers_ConflictsReportShowsLower(_Installers_BoolLink_Refresh):
    ...
    help = _('In the conflicts tab also display conflicts with lower order '
...
class Installers_ConflictsReportShowBSAConflicts(_Installers_BoolLink_Refresh):
    ...
    help = _('In the conflicts tab also display same-name resources inside '
...
class Installers_AvoidOnStart(BoolLink):
    ...
    _text, key, help = _('Avoid at Startup'), 'bash.installers.fastStart', _(
...
class Installers_RemoveEmptyDirs(BoolLink):
    ...
    _text, key = _('Clean Data Directory'), 'bash.installers.removeEmptyDirs'
...
class Installers_SortActive(_Installer_Sort, BoolLink):
    """Sort by type."""
    _text, key, help = _('Sort by Active'), 'bash.installers.sortActive', _(
...
class Installers_SortProjects(_Installer_Sort, BoolLink):
    ...
    _text, key, help = _('Projects First'), 'bash.installers.sortProjects', _(
...
class _Installers_SkipOBSEPlugins(AppendableLink, _Installers_Skip):
    ...
    _text = _('Skip %s Plugins') % bush.game.se_sd
