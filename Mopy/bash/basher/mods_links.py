...
class Mods_LoadList(ChoiceLink):
    ...
    def __init__(self):
        ...
        class _SaveLink(EnabledLink):
            ...
            help = _('Save the currently active mods to a new active mods list')
...
class Mods_EsmsFirst(CheckLink, EnabledLink):
    ...
    help = _('Sort masters by type. Always on if current sort is Load Order.')
    _text = _('Type')
...
class Mods_SelectedFirst(CheckLink):
    ...
    help = _('Sort loaded mods to the top')
    _text = _('Selection')
...
class Mods_OblivionVersion(CheckLink, EnabledLink):
    ...
    help = _('Specify/set Oblivion version')
...
class Mods_CreateBlankBashedPatch(ItemLink):
    ...
    _text, help = _('New Bashed Patch...'), _('Create a new bashed patch')
...
class Mods_CreateBlank(ItemLink):
    ...
    _text, help = _('New Mod...'), _('Create a new blank mod')
    ...
    def __init__(self, masterless=False):
        ...
        if masterless:
            ...
            self.help = _('Create a new blank mod with no masters')
...
class Mods_ListMods(ItemLink):
    ...
    _text = _("List Mods...")
    help = _("Copies list of active mod files to clipboard.")

    def Execute(self):
        #--Get masters list
        text = bosh.modInfos.getModList(showCRC=balt.getKeyState(67))
        balt.copyToClipboard(text)
        self._showLog(text, title=_("Active Mod Files"), fixedFont=False)
...
class Mods_CleanDummyMasters(EnabledLink):
    ...
    help = _("Clean up after using a 'Create Dummy Masters...' command")
...
class Mods_ScanDirty(BoolLink):
    ...
    help = _("Display a tooltip if mod is dirty and underline dirty mods - "
...
class Mods_LockLoadOrder(CheckLink):
    ...
    help = _("Will reset mod Load Order to whatever Wrye Bash has saved for"
...
class Mods_CrcRefresh(ItemLink):
    ...
    help = _('Clean stale CRCs from cache')
