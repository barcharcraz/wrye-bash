...
def generateTweakLines(wizardEdits, target):
    ...
    for realSection in wizardEdits:
        if realSection == ']set[':
            modFormat = wizardEdits[realSection][0]+' %(setting)s to %(value)s%(comment)s'
            delFormat = ';-'+wizardEdits[realSection][0]+' %(setting)s to DELETED'
        elif realSection == ']setgs[':
            modFormat = wizardEdits[realSection][0]+' %(setting)s %(value)s%(comment)s'
            delFormat = ';-'+wizardEdits[realSection][0]+' %(setting)s DELETED'
        elif realSection == ']SetNumericGameSetting[':
            modFormat = wizardEdits[realSection][0]+' %(setting)s %(value)s%(comment)s'
            delFormat = ';-'+wizardEdits[realSection][0]+' %(setting)s DELETED'
...
class PageFinish(PageInstaller):
    def __init__(self, parent, subsList, espmsList, espmRenames, bAuto, notes, iniedits):
        ...
        espmSizer = balt.vSizer(balt.StaticText(self, _('Esp/ms')), vspace(2))
...
class PageVersions(PageInstaller):
    def __init__(self, parent, bGameOk, gameHave, gameNeed, bSEOk, seHave,
        ...
        bmp = [wx.Bitmap(bass.dirs['images'].join('x.png').s),
               wx.Bitmap(bass.dirs['images'].join('check.png').s)
               ]
...
class WryeParser(ScriptParser.Parser):
    ...
    def fnDisableINILine(self, iniName, section, setting):
        ...
        self.iniedits.setdefault(iniPath, bolt.LowerDict()).setdefault(realSection,[section,bolt.LowerDict()])
        self.iniedits[iniPath][realSection][0] = section
        self.iniedits[iniPath][realSection][1][setting] = (setting,'','',True)
    ...
    def _SelectAll(self, bSelect):
        ...
        for i in list(self.espmlist.keys()):
